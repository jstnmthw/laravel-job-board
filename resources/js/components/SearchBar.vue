<template>
    <form class="flex" v-on:submit.prevent="onSubmit()">
        ...
        <div class="relative focus-within:bg-white focus-within:bg-gray-100 text-gray-400 focus-within:text-gray-300 pl-6 border border-gray-300 border-r-0 bg-gray-100 w-[185px] focus:bg-white focus:border-blue-200 focus:ring focus:ring-blue-100 focus:ring-opacity-50">
            ...
            <input :value="location.name" @input="searchLocation" type="text" required class="text-[15px] text-gray-800 bg-transparent border-0 w-full focus:border-0 focus:ring-0 placeholder-gray-400 focus:placeholder-gray-300" placeholder="Location">
    ...
    methods: {
        close() {
            ...
            if (!this.location.id && this.locationSearchResults) {
                this.$store.commit('search/SET_SEARCH', { location: this.locationSearchResults[0]._source })
            }
        },
        clearLocation() {
            this.close()
            this.$store.commit('search/SET_SEARCH', { location: {} })
        },
        onSubmit() {
            this.$router.push({
                path: '/jobs',
        ...
        async searchLocation(e) {
            this.$store.commit('search/SET_SEARCH', { location: { name: e.target.value }})
